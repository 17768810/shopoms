...
func (this *MainController) Profile() {
	...
	if this.isPost() {
		...
		user.Weixin = this.GetString("weixin")
		...
		if password1 != "" {
			if len(password1) < 6 {
				...
				user.Password = libs.GetPassword(password1, user.Salt)
				user.Update()
			}
		}
		flash.Success("修改成功！")
...
// 绑定微信
func (this *MainController) BindWeixin() {
	beego.ReadFromRequest(&this.Controller)
	user, _ := models.UserGetById(this.userId)
	...
	if this.isPost() {
		...
		if password1 != "" {
			if len(password1) < 6 {
				flash.Error("密码长度必须大于6位")
				flash.Store(&this.Controller)
				this.redirect(beego.URLFor(".BindWeixin"))
			} else if password2 != password1 {
				flash.Error("两次输入的密码不一致")
				flash.Store(&this.Controller)
				this.redirect(beego.URLFor(".BindWeixin"))
	...
	if this.isPost() {
		...
		if password1 != "" {
			if len(password1) < 6 {
				flash.Error("密码长度必须大于6位")
				flash.Store(&this.Controller)
				this.redirect(beego.URLFor(".Setting"))
			} else if password2 != password1 {
				flash.Error("两次输入的密码不一致")
				flash.Store(&this.Controller)
				this.redirect(beego.URLFor(".Setting"))
...
func (this *MainController) Login() {
	...
	if this.isPost() {
		...
		if username != "" && password != "" {
			...
			errorMsg := ""
			md5Password := libs.GetPassword(password, user.Salt)
